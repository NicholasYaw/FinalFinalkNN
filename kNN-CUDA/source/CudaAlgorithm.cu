...
void Cuda::knn(const FlatDataView& trainingData, FlatDataView& testingData)
{
    ...
    knnOnCuda<<<NUM_OF_BLOCKS, NUM_OF_THREADS>>>(deviceTrainingData, deviceTestingData, 
        trainingData.getNumberOfRows(), testingData.getNumberOfRows(), testingData.getRowSize());
